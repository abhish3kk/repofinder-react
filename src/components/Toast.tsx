...
const Toast = () => {
  ...
  useEffect(() => {
    state.notifications.forEach((notification) => {
      setTimeout(() => {
        setTimeout(() => dispatch({ type: "REMOVE_NOTIFICATION", payload: notification.id }), 500); 
      }, 3000); 
    });
    ...
    <div className="fixed right-4 top-4 space-y-2">
      {state.notifications.map((notification, index) => (
        <div 
          ...
          role="alert">
          <div className={`inline-flex items-center justify-center shrink-0 w-8 h-8 text-green-500 bg-green-100 rounded-lg dark:bg-green-800 dark:text-green-200 ${notificationClass(notification)}`}>
            { notification.type === "success" && <CircleCheck />}
            { notification.type === "error" && <CircleX />}
            { notification.type === "info" && <Info />}
          </div>
          <div className="ms-3 text-sm font-normal me-3">{notification.message}</div>
          <button type="button" className="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700 cursor-pointer" onClick={() => dispatch({type: "REMOVE_NOTIFICATION", payload: notification.id})}>
